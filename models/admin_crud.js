...
async function admin_crud(req_body, table_name) {
  ...
  let fullname = req_body.fullname;
  ...
  if (btn == "Update"){
    let query_string = {
      text: `UPDATE ${table_name} SET username=$1, password=$2, fullname=$3, role=$4, department_id=$5 WHERE id=$6`,
      values: [username, password, fullname, role, department_id, id],
    }
    ...
    let query_string = {
      text: `INSERT INTO ${table_name} VALUES ($1, $2, $3, $4, $5, $6)`,
      values: [id, username, password, fullname, role, department_id],
    }
    await client.query(query_string)
    await client.end();
  }
